...
<head>
    ...
    <title>Admin Login | Janisha Jaya</title>
...
<body class="min-h-screen flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-sm">
        ...
        <div class="bg-[#151f37] border border-white/5 rounded-xl p-8 shadow-2xl">
            <div class="mb-6">
                <h2 class="text-lg font-semibold text-white" style="font-family:'Hanken Grotesk',sans-serif;">Admin Login</h2>
                <p class="text-xs text-[#bacac3] mt-1">Sign in to manage your portfolio</p>
            </div>

            <?php if (!empty($error)): ?>
            <div class="mb-5 flex items-start gap-3 p-3 rounded-lg bg-red-900/20 border border-red-500/20 text-[#ffb4ab] text-sm">
                <span class="material-symbols-outlined text-[18px] flex-shrink-0 mt-0.5">error</span>
                <div><?= $error ?></div>
            </div>
            <?php endif; ?>

            <form method="post" action="<?= base_url('administrator/login') ?>" class="space-y-4">
                <div>
            ...
            <div class="mt-6 pt-5 border-t border-white/5 text-center space-y-2">
                <p class="text-xs text-[#bacac3]">
                    No account?
                    <a href="<?= base_url('administrator/register') ?>" class="text-[#38debb] hover:underline">Register here</a>
                </p>
                <a href="<?= base_url('home') ?>" class="block text-xs text-[#bacac3] hover:text-[#38debb] transition-colors" style="font-family:'JetBrains Mono',monospace;">
                    ← Back to Portfolio
                </a>
            </div>
        </div>
